...
router.post('/cover-letter', async (req: AuthRequest, res) => {
  try {
    ...
    const { data } = await supabase
      .from('applications')
      .insert({ user_id: req.userId, job_url: 'manual_job', status: 'generated', response: out.slice(0, 2000) })
      .select('id')
      .single();

    res.json({ coverLetter: out, applicationId: data?.id });
  } catch (e: any) {
